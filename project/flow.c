...
struct tcp_flow flow_linger[FLOWS_KEEP_LINGER];
int flow_count = 0;
// int flow_head = 0;
// int flow_tail = 0;
	...
	if (key->eth.type == htons(ETH_P_IP)) {
		...
		if (key->ip.proto == IPPROTO_TCP) {
			...
			if (tcphdr_ok(skb) && (eth != NULL)) {
				...
				// printk(KERN_DEBUG " %d %d outside", tcp->source, tcp->dest);
				if (cond == 0)
				{
					__u16 wscale = 0;
					int condition = 0, i = 0;
					...
					if(flow_count == 0)//no data in flow_linger
					{
						ttt = flow_linger + 0;
						ttt->ip_src = wz_linger.ip_src;
						ttt->ip_dst = wz_linger.ip_dst;
						ttt->sourceaddr = wz_linger.sourceaddr;
						ttt->destination = wz_linger.destination;
						ttt->winsize = wz_linger.windowsize;
						ttt->wscale = wscale;
						ttt->sec = wz_linger.sec;
						ttt->usec = wz_linger.usec;
						memcpy(ttt->eth_src, eth->h_source, 6);
						memcpy(ttt->eth_dst, eth->h_dest, 6);
						ttt->packets = 1;
						flow_count = 1;
					}
					else//thereis data, find its corresponding flow, update, or find new location,insert
					{
						int insert_linger = 0;
						for(i = 0; i < FLOWS_KEEP_LINGER; i++) // try to find its corresponding flow.
						{
							ttt = flow_linger + i;
						...
						if(insert_linger == 0) // not found, get new insert location.
						{
							for(i = 0; i < FLOWS_KEEP_LINGER; i++) // try to find its corresponding flow.
							{
								ttt = flow_linger + i;
								if(ttt->sec == 0)//get the new location.
								{
									ttt->ip_src = wz_linger.ip_src;
									ttt->ip_dst = wz_linger.ip_dst;
									ttt->sourceaddr = wz_linger.sourceaddr;
					...
					for(i = 0; i < FLOWS_KEEP_LINGER; i++) // del outdate items.
					{
						ttt = flow_linger + i;
						if((wz_linger.sec - ttt->sec) > 2)
						{
							memset(ttt, 0, sizeof(struct tcp_flow));
							flow_count -= 1;
						}
					}
