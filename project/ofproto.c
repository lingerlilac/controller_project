...
void assign_string(struct item_value* d, int s);
...
void set_cwnd(struct tcp_flow* tflow, char *ret, int size, int window)
{
  ...
  unsigned char *ptr_uc;
        reversebytes_uint32t(&(tflow->ip_src));
        reversebytes_uint32t(&(tflow->ip_dst));
  ...
  memset(buff, 0, 200);
  sprintf(buff, "ovs-ofctl add-flow br0 -O openflow13 tcp,nw_src=%s,nw_dst=%s,tcp_src=%d,tcp_dst=%d,actions=mod_tp_dst:%d",
...
void run_linger(struct tcp_ss *str)
{
    ...
    {
        VLOG_INFO("socket create failed\n");
        // return -1;
    }
...
int main_linger(void)
{
  ...
  else
  {
    ...
    struct linkstates link_info[TEN];
    ...
    VLOG_INFO("xxx %d\n", apst.station_count);
    for(j = 0; j < apst.station_count; j++)
    {
      if(apst.station_count < TEN)
      {
        struct linkstates *tmp = link_info + j;
        memset(&info, 0, sizeof(info));
        ret = recvfrom(skfd, &info, sizeof(struct _my_msg), 0, (struct sockaddr *)&dest_addr, (socklen_t*)sizeof(dest_addr));
        memset(tmp, 0, sizeof(struct linkstates));
        memcpy(tmp, info.data, sizeof(struct linkstates));
        count_link += 1;
      }
      else
      {
    ...
    for(j = 0; j < count_link; j++)
    {
      struct linkstates lkst;
      // memset(&info, 0, sizeof(info));
      // ret = recvfrom(skfd, &info, sizeof(struct _my_msg), 0, (struct sockaddr *)&dest_addr, (socklen_t*)sizeof(dest_addr));
      memset(&lkst, 0, sizeof(struct linkstates));
      // memcpy(&lkst, info.data, sizeof(struct linkstates));
      lkst = link_info[j];
      fstates[congestion].value = ((float)apst.congestion) * times;
      assign_string(&fstates[congestion], congestion);
      fstates[noncongestion].value = ((float)apst.noncongestion) * times;
      assign_string(&fstates[noncongestion], noncongestion);
      fstates[station_count].value = ((float)apst.station_count);
      assign_string(&fstates[station_count], station_count);
      fstates[station_count].value = ((float)apst.neibours);
      assign_string(&fstates[neibours], neibours);
      ...
      fstates[q3requeues].value = ((float)apst.q3requeues * times);
      ...
      fstates[qfrequeues].value = ((float)apst.qfrequeues * times);
      assign_string(&fstates[qfrequeues] , qfrequeues);


      fstates[signal].value = (float)lkst.signal;
      assign_string(&fstates[signal] , signal);
      fstates[signal_avg].value = (float)lkst.signal_avg;
      assign_string(&fstates[signal_avg] , signal_avg);
      fstates[qfrequeues].value = ((float)lkst.inactive_time * times);
      assign_string(&fstates[inactive_time] , inactive_time);
      fstates[qfrequeues].value = ((float)lkst.rx_bytes * times);
      assign_string(&fstates[rx_bytes] , rx_bytes);
      fstates[qfrequeues].value = ((float)lkst.rx_packets * times);
      assign_string(&fstates[rx_packets] , rx_packets);

      fstates[qfrequeues].value = ((float)lkst.tx_bytes * times);
      assign_string(&fstates[tx_bytes] , tx_bytes);
      fstates[qfrequeues].value = ((float)lkst.tx_packets * times);
      assign_string(&fstates[tx_packets] , tx_packets);

      fstates[qfrequeues].value = ((float)lkst.tx_retries * times);
      assign_string(&fstates[tx_retries] , tx_retries);
      fstates[qfrequeues].value = ((float)lkst.tx_failed * times);
      assign_string(&fstates[tx_failed] , tx_failed);

      fstates[station_count].value = ((float)lkst.expected_throughput);
      assign_string(&fstates[expected_throughput], expected_throughput);
    
      for (i = 1; i < 31; i++)
      {
        VLOG_INFO("ddd %d: %f\t", fstates[i].name, fstates[i].value);
        if(!(i % 4))
          VLOG_INFO("\n");
      }
      // VLOG_INFO("begin ----------------------------\n");
      search_results = bst_search(tnode, fstates, 31);
      // VLOG_INFO("over -----------------------------\n");
      // search_results = 1;
      if (search_results)
      {
        ...
        for(i = 0; i < TEN; i++)
        {
          ...
          if(tmp.packets <= 0)
            continue;
          VLOG_INFO("sssssssssssssssssss");
          found = strcmp_linger(tmp.eth_src, lkst.station) + strcmp_linger(tmp.eth_dst, lkst.station);
          print_mac((unsigned char*)tmp.eth_src);
          print_mac((unsigned char*)tmp.eth_dst);
          print_mac((unsigned char*)lkst.station);
          VLOG_INFO("found is %d", found);
          if (found == 1)
          {
            // char *ret1 = NULL;
            char *find_192 = NULL;
            struct tcp_ss *tcpinfo = NULL;
            int length_ss = sizeof(struct tcp_ss);
            char ip_src[20];
            char ip_dst[20];
            char ip_src1[20];
            reversebytes_be16t(&(tmp.sourceaddr));
            reversebytes_be16t(&(tmp.destination));
            // int port = tmp.sourceaddr;

            unsigned char *ptr_uc = NULL;
            memset(ip_src, 0, 20);
            memset(ip_dst, 0, 20);
            memset(ip_src1, 0, 20);
            reversebytes_uint32t(&(tmp.ip_src));
            reversebytes_uint32t(&(tmp.ip_dst));
            ptr_uc = (unsigned char *) &tmp.ip_src;
            sprintf (ip_src, "%u.%u.%u.%u", ptr_uc[3], ptr_uc[2], ptr_uc[1], ptr_uc[0]);
            sprintf (ip_src1, "%u.%u.%u.%u", ptr_uc[0], ptr_uc[1], ptr_uc[2], ptr_uc[3]);
            ptr_uc = (unsigned char *) &tmp.ip_dst;
            sprintf (ip_dst, "%u.%u.%u.%u", ptr_uc[3], ptr_uc[2], ptr_uc[1], ptr_uc[0]);
            // VLOG_INFO("%d %d", cwnd, mss);
            VLOG_INFO("%s %llu", ip_src, tmp.packets);
            find_192 = strpbrk(ip_src, "192.168.11.");
            if(find_192 != NULL)
            {
              tcpinfo = (struct tcp_ss *)malloc(length_ss);
              memset(tcpinfo, 0, length_ss);
              memcpy(tcpinfo->ip_src, ip_src, 20);
              memcpy(tcpinfo->ip_dst, ip_dst, 20);
              tcpinfo->src_port = tmp.sourceaddr;
              tcpinfo->dst_port = tmp.destination;
              run_linger(tcpinfo);
              free(tcpinfo);
              tcpinfo = NULL;
            }
...
struct treenode* bst_search(struct treenode* node, struct item_value value[], int length)
{
  ...
  while (node && (node->name != 0))
  {
    ...
    for (i = 1; i < length; i++)
    {
      VLOG_INFO("%d %d", node->name, value[i].name);
      if (node->name == value[i].name)
      {
        found = 1;
        break;
      }
    ...
    if (found == 0)
    {
      VLOG_INFO("-xxxxxxxxxxxxxxxxxxxxxx\n");
      return NULL;
    }
  ...
  if (found)
  {
    VLOG_INFO("1 name %d value %f\n", node_pre->name, node_pre->value);
    return node;
  }
  else
  {
    VLOG_INFO("2 name %d value %f\n", node_pre->name, node_pre->value);
    return node_pre;
  }
}
...
int create_items(struct item_value items_list[], char *buffer)
{
  ...
  for (j = 0; j < in; j++)
  {
    if (j % 2 ==  0)
    {
      items_list[index].name = atoi(p[j]);
    }
    else
    {
...
static enum ofperr handle_experimenter(const struct ofpbuf *msg)
{
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    case OFPTYPE_EXPERIMENTER:
        ...
        // handle_msg(msg);
        // handle_msg();
        return handle_experimenter(msg);
