...
void inline iw2iwp(struct iw* p, struct iwp* q);
/**
 * APIs
 */
...
void inline iw2iwp(struct iw* q, struct iwp* p)
{
    if((p != NULL) && (q != NULL))
    {
        p->timex = (__be64)(q->sec) * 1000 + (__be64)q->usec;
        p->inactive_time = q->inactive_time;
        p->rx_bytes = q->rx_bytes;
        p->rx_packets = q->rx_packets;
        p->tx_bytes = q->tx_bytes;
        p->tx_packets = q->tx_packets;
        p->tx_retries = q->tx_retries;
        p->tx_failed = q->tx_failed;
        p->signal = q->signal;
        p->signal_avg = q->signal_avg;
        p->expected_throughput = float(q->expected_throughput) / 1000.0;
        p->next = NULL;
    }
}
void *
...
decode_iw (char *buf)
{
    ...
    if (station_lists == NULL)
    {
        ...
        iwtmp = station_lists->iwh;
        iwtmp = (struct data_iwp *)malloc(sizeof(struct data_iwp));
        memset(iwtmp, 0, sizeof(struct data_iwp));
        iw2iwp(&rdata, iwtmp);
    }
    else
    {
        ...
        while (tlist) // remove the old ones.
        {
            if ((rdata.sec - tlist->sec) > 10) //out of time item, delete.
            {
                ...
                if (tlist == station_lists) // head
                {
                    ...
                    {
                        struct data_iwp * iwptr = NULL;
                        iwptr = station_lists->iwh;
                        while(iwptr)
                        {
                            struct data_iwp *tmp1 = NULL;
                            tmp1 = iwptr;
                            iwptr = iwptr->next;
                            free(tmp1);
                            tmp1 = NULL;
                ...
                else if (tlist == station_l_last) // tail
                {
                    ...
                    struct data_iwp *iwptr = station_l_last;
                    tmp1 = station_lists;
        ...
        while (tlist) // insert.
        {
            ...
            {
                __be64 time_topptr = 0;
                found = 1;
                tlist->sec = rdata.sec;
                time_topptr = tlist->iwh->timex;
                if((timex - time_topptr) > 900) //when a new item will be added to list, if time duration is over threshold, compute corresponding
                    //results.

                {
                    float duration = 0.0;
                    duration = (float)(timex - time_topptr);
                    tlist->iwget.inactive_time = (float)(rdata.inactive_time - tlist->iwh->inactive_time) / duration;
                    tlist->iwget.rx_bytes = (float)(rdata.rx_bytes - tlist->iwh->rx_bytes) / duration;
                    tlist->iwget.rx_packets = (float)(rdata.rx_packets - tlist->iwh->rx_packets) / duration;
                    tlist->iwget.tx_bytes = (float)(rdata.tx_bytes - tlist->iwh->tx_bytes) / duration;
                    tlist->iwget.tx_packets = (float)(rdata.tx_packets - tlist->iwh->tx_packets) / duration;
                    tlist->iwget.tx_retries = (float)(rdata.tx_retries - tlist->iwh->tx_retries) / duration;
                    tlist->iwget.tx_failed = (float)(rdata.tx_failed - tlist->iwh->tx_failed) / duration;
                    tlist->iwget.signal = rdata.signal;
                    tlist->iwget.signal_avg = rdata.signal_avg;
                    tlist->iwget.expected_throughput = expected_throughput;
                }
                break;
            }
            tlist = tlist->next;
            ...
            while (amount > 0)
            {
                ...
                if (time_tp.sec) //compute statistic when time_tp
                {
                    ...
                    if ((s_keep.sec != 0) && (q_keep_3.sec != 0))
                    {
                        ...
                        int stations = 0;
                        int duration = 0;
                        struct data_iwget iwget_linger;
                        ...
                        memset(&iwget_linger, 0, sizeof(struct data_iwget));
                        if (station_lists)
                        {
                            iwget_linger = station_lists->iwget;
