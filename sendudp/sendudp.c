  ...
  if (skb->len >= nlmsg_total_size(0)) {
    ...
    if (!strcmp_n(data, "states", 6)) {
      ...
      if ((duration == 0) || (dur_sur == 0)) {
        ...
        apstat.drop_count =
            ((tmp2.drop_count - tmp1.drop_count) * times) / duration;

        apstat.time_busy =
            (times * (tmp2.time_busy - tmp1.time_busy)) / dur_sur;
        apstat.time_ext_busy =
            (times * (tmp2.time_ext_busy - tmp1.time_ext_busy)) / dur_sur;
        apstat.time_rx = (times * (tmp2.time_rx - tmp1.time_rx)) / dur_sur;
        apstat.time_tx = (times * (tmp2.time_tx - tmp1.time_tx)) / dur_sur;
        apstat.time_scan =
            (times * (tmp2.time_scan - tmp1.time_scan)) / dur_sur;
        apstat.noise = tmp2.noise;

        apstat.q3packets =
            (times * (tmp2.q3packets - tmp1.q3packets)) / duration;
        apstat.q3bytes = (times * (tmp2.q3bytes - tmp1.q3bytes)) / duration;
        apstat.q3qlen = q3maxqlen;
        apstat.q3backlog = q3maxbacklog;
        apstat.q3drops = (times * (tmp2.q3drops - tmp1.q3drops)) / duration;
        apstat.q3requeues =
            (times * (tmp2.q3requeues - tmp1.q3requeues)) / duration;
        apstat.q3overlimits =
            (times * (tmp2.q3overlimits - tmp1.q3overlimits)) / duration;

        apstat.qfpackets =
            (times * (tmp2.qfpackets - tmp1.qfpackets)) / duration;
        apstat.qfbytes = (times * (tmp2.qfbytes - tmp1.qfbytes)) / duration;
        apstat.qfqlen = qfmaxqlen;
        apstat.qfbacklog = qfmaxbacklog;
        apstat.qfdrops = (times * (tmp2.qfdrops - tmp1.qfdrops)) / duration;
        apstat.qfrequeues =
            (times * (tmp2.qfrequeues - tmp1.qfrequeues)) / duration;
        apstat.qfoverlimits =
            (times * (tmp2.qfoverlimits - tmp1.qfoverlimits)) / duration;
