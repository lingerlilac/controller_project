...
static void recv_cb(struct sk_buff *skb)
{
    ...
    if(skb->len >= nlmsg_total_size(0))
    {
        ...
        else if(!strcmp(data, "states"))
        {
    		...
    		xyz = (tmp2.sec - tmp1.sec) * 1000 + (tmp2.usec - tmp1.usec) / 1000;
    		printk(KERN_DEBUG "%u %u %u\n", tmp2.sec, tmp1.sec, xyz);
    		duration = xyz;
    		...
    		if((duration == 0) || (dur_sur == 0))
    		{
    			...
    			// printk(KERN_DEBUG "divide zero\n");
    		}
    		else
    		{
    			int length = 0;
    			length = sizeof(struct apstates);
    			printk(KERN_DEBUG "%u %u %u %llu %llu %llu %d\n", tmp2.sec, tmp2.usec, tmp2.drop_count, tmp2.time, tmp2.time_busy, tmp2.time_ext_busy, length);
    			printk(KERN_DEBUG "%u %u %u %llu %llu %llu %d\n", tmp1.sec, tmp1.usec, tmp1.drop_count, tmp1.time ,tmp1.time_busy, tmp1.time_ext_busy, length);
	    		apstat.drop_count = ((tmp2.drop_count - tmp1.drop_count) * 1000) / duration;
	    		
	    		apstat.time_busy = (1000000 * (tmp2.time_busy - tmp1.time_busy)) / dur_sur;
	    		apstat.time_ext_busy = (1000000 * (tmp2.time_ext_busy - tmp1.time_ext_busy)) / dur_sur;
	    		apstat.time_rx = (1000000 * (tmp2.time_rx - tmp1.time_rx)) / dur_sur;
	    		apstat.time_tx = (1000000 * (tmp2.time_tx - tmp1.time_tx)) / dur_sur;
	    		apstat.time_scan = (1000000 * (tmp2.time_scan - tmp1.time_scan)) /dur_sur;
	    		apstat.noise = tmp2.noise;
	    		apstat.packets = (1000000 * (tmp2.packets - tmp1.packets)) / duration;
	    		apstat.bytes = (1000000 * (tmp2.bytes - tmp1.bytes)) / duration;
	    		apstat.qlen = tmp2.qlen;
	    		apstat.backlog = tmp2.backlog;
	    		apstat.drops = (1000000 * (tmp2.drops - tmp1.drops)) / duration;
	    		apstat.requeues = (1000000 * (tmp2.requeues - tmp1.requeues)) / duration;
	    		apstat.overlimits = (1000000 * (tmp2.overlimits - tmp1.overlimits)) / duration;
	    		apstat.sec = tmp2.sec;
	    		apstat.usec = tmp2.usec;
	    		// printk(KERN_DEBUG "eeeeffff\n");
	    		buf = (char *)kmalloc(length, GFP_KERNEL);
	    		// printk(KERN_DEBUG "helllllo\n");
	    		memset(buf, 0, length);
